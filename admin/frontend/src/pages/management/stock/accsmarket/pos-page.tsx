...
export default function AccsmarketPosPage() {
  ...
  useEffect(() => {
    api.get("/management/accsmarkets/search/customers").then((r) => setCustomers(r.data?.customers ?? [])).catch(() => {})
  }, [])

  useEffect(() => {
    const t = setTimeout(() => {
      if (customerSearchQuery) {
        api.get("/management/accsmarkets/search/customers", { params: { search: customerSearchQuery } })
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <form onSubmit={handleSubmit} className="flex-1 flex flex-col sm:grid sm:grid-cols-[1fr_380px] gap-6 min-h-0">
        {/* LEFT — Account list (order-2 on mobile) */}
        ...
        {/* RIGHT — Checkout panel (order-1 on mobile) */}
          ...
          <Card className="overflow-visible">
            ...
            <CardContent className="px-4 pb-4 space-y-3 overflow-visible">
              ...
              <div className="flex items-center gap-2.5">
                <Checkbox id="shopee" checked={isShopee} onCheckedChange={(v) => { setIsShopee(!!v); if (!v) setShopeeOrderNumber("") }} />
