...
import { RefreshCw, Clock, Search, Users, LogIn, Mail, CheckCircle2 } from "lucide-react"
...
interface JobAccountsResponse {
  ...
  employees: string[]
  stats: { total_accounts: number; login_only: number; email_replacement: number; selesai: number }
}
      ...
      <Card className="overflow-hidden p-0 hidden sm:block">
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {isLoading ? <LoadingRow colSpan={6} /> : !accounts.length ? (
                <EmptyRow colSpan={6} message="Belum ada data job" />
              ) : (
                accounts.map((a) => (
                  <TableRow key={a.id}>
