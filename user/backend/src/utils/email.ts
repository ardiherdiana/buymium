...
export async function sendOtpEmail(opts: {
  ...
  const body = `
    <p>Halo <strong>${opts.name}</strong>,</p>
    <p>Gunakan kode OTP berikut untuk menyelesaikan pendaftaran akun Buymium kamu:</p>
    <div style="margin:24px auto;text-align:center;">
      <div style="display:inline-block;background:#f1f5f9;border:2px dashed #cbd5e1;border-radius:12px;padding:16px 40px;">
        <span style="font-size:36px;font-weight:800;letter-spacing:12px;color:#0f172a;font-family:monospace;">${opts.otp}</span>
      </div>
    </div>
    <p style="text-align:center;color:#6b7280;font-size:13px;">Kode berlaku selama <strong>10 menit</strong>. Jangan bagikan ke siapapun.</p>
    <p style="background:#fef3c7;border:1px solid #fcd34d;padding:12px;border-radius:8px;color:#92400e;font-size:13px;margin-top:16px;">
      Jika kamu tidak mendaftar di Buymium, abaikan email ini.
    </p>
  `
  await send({
    to: opts.to,
    subject: `${opts.otp} — Kode verifikasi Buymium`,
    html: wrapper('Verifikasi Email', body),
  })
}
...
export async function sendOrderCreated(opts: {
  ...
  const body = `
    <p>Halo <strong>${opts.name}</strong>,</p>
    <p>Pesanan kamu telah kami terima dengan rincian:</p>
    <table cellpadding="6" cellspacing="0" style="margin-top:8px;border-collapse:collapse;width:100%;">
      <tr><td style="color:#6b7280;width:40%;">Nomor Order</td><td style="font-weight:600;">${opts.groupId}</td></tr>
      <tr><td style="color:#6b7280;">Jumlah Item</td><td>${opts.itemCount} akun</td></tr>
      <tr><td style="color:#6b7280;">Total</td><td style="font-weight:700;color:#16a34a;">${fmtRp(opts.totalPrice)}</td></tr>
    </table>
    <p style="margin-top:16px;">Silakan lakukan transfer ke rekening yang tertera, lalu upload bukti transfer di halaman pesanan. Pesanan akan otomatis dibatalkan dalam 24 jam jika belum dibayar.</p>
  `
  await send({
    to: opts.to,
    subject: `Pesanan ${opts.groupId} dibuat — menunggu pembayaran`,
    html: wrapper('Pesanan Dibuat', body, `${SITE_URL}/orders`, 'Lihat Pesanan'),
  })
}

export async function sendProofUploaded(opts: {
  to: string
  name: string
  groupId: string
  totalPrice: number
  ...
  const body = `
    <p>Halo <strong>${opts.name}</strong>,</p>
    <p>Bukti transfer untuk pesanan <strong>${opts.groupId}</strong> sebesar <strong>${fmtRp(opts.totalPrice)}</strong> telah kami terima.</p>
    <p>Tim kami sedang memverifikasi. Setelah dikonfirmasi (biasanya &lt;1 jam pada jam kerja), kamu akan menerima email berisi link untuk download data akun.</p>
  `
  await send({
    to: opts.to,
    subject: `Bukti transfer diterima — ${opts.groupId}`,
    html: wrapper('Bukti Transfer Diterima', body, `${SITE_URL}/orders`, 'Cek Status Pesanan'),
  })
}
...
export async function sendOrderConfirmed(opts: {
  ...
  const body = `
    <p>Halo <strong>${opts.name}</strong>,</p>
    <p>🎉 Pembayaranmu untuk pesanan <strong>${opts.groupId}</strong> telah dikonfirmasi.</p>
